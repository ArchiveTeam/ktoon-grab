...
More information about the archiving project can be found on the ArchiveTeam wiki: [케이툰](https://wiki.archiveteam.org/index.php?title=케이툰)
...
after which the project container can be run:
    ...
    docker run -d --name archiveteam --label=com.centurylinklabs.watchtower.enable=true --log-driver json-file --log-opt max-size=50m --restart=unless-stopped atdr.meo.ws/archiveteam/ktoon-grab --concurrent 1 YOURNICKHERE
